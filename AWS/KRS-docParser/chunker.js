 ...
 *
 * Why word-based rather than character-based?
 * - Embedding models (Vertex textembedding-gecko) have token limits, not char limits
 * - Word count is a close-enough proxy without a full tokeniser
 * - Overlap preserves sentence context across chunk boundaries so semantic
 *   search doesn't miss content that straddles a boundary
 *
 * @param {string} rawText
 * @param {number} chunkSize    - target words per chunk (default from ENV)
 * @param {number} overlapWords - words to repeat from previous chunk (default from ENV)
 * @returns {Array<{ chunkIndex: number, text: string, wordCount: number, startWord: number, endWord: number }>}
 */
