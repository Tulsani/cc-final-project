 ...
 *
 * Receives Pub/Sub push messages via HTTP POST.
 * Each message = one chunk from the textract-processor.
 *
 * MAP pattern:
 *   Pub/Sub push subscription delivers one HTTP request per message.
 *   Cloud Run spawns one instance per concurrent request.
 *   With max-instances set, this is bounded parallel embedding — pure map.
 *   Scale-to-zero: when the Pub/Sub subscription has no messages,
 *   no HTTP requests arrive, Cloud Run scales to 0 automatically.
 *
 * Processing per message:
 *   1. Decode Pub/Sub envelope → chunk SQS body
 *   2. Fetch chunk text from AWS S3 (cross-cloud read)
 *   3. Embed text via Vertex AI textembedding-gecko@003
 *   4. Upsert vector → Vertex AI Vector Search
 *   5. Write metadata → Firestore
 *   6. Return HTTP 200 → Pub/Sub ACKs the message
 *      Return HTTP 4xx/5xx → Pub/Sub NACKs and redelivers (retry)
 */
