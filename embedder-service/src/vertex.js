...
/**
 * Generates a single embedding for one text string.
 * Cloud Run embedder processes one Pub/Sub message (one chunk) per instance,
 * so we don't need batching here — each invocation embeds exactly one chunk.
 *
 * task_type RETRIEVAL_DOCUMENT = optimised for storage-side vectors.
 * At query time we use RETRIEVAL_QUERY instead.
 *
 * @param {string} text
